...
class AptitudeController extends Controller
{
    ...
    {
        $data = Aptitude::with(['applicant.regions', 'applicant.branches'])->get();
    ...
    //     // return redirect()->route('test.applicant-aptitude-test')->with('success', 'status saved successfully.');
    // }
    ...
    public function store_applicant_aptitude(Request $request, $uuid)
    {
        ...
        if ($validatedData['aptitude_status'] === 'QUALIFIED') {
            ...
            // If no BasicFitness record exists, create a new one
            if (!$basicFitness) {
                BasicFitness::create([
                    'applicant_id' => $applied_applicant->id,
                    // Add any other fields you want to save in BasicFitness
                ]);
            }
        }
    ...
    public function confirm_applicant_aptitude(Request $request, $uuid)
    {
        ...
        if ($request->aptitude_status === 'DISQUALIFIED' && $applied_applicant->aptitude_status === 'QUALIFIED') {
            ...
            $basicFitness = BasicFitness::where('applicant_id', $applied_applicant->applicant_id)->first();
            if ($basicFitness) {
                $$basicFitness->delete();
            }
        }
